...
public class QuNumberCounter {
	...
	public static void main(String[] args) {
		...
		int[] answer = { 1,4,4,3,1,4,4,2,1,3,2  };//선언과 동시에 초기화(결과값 채워짐)
		/*
		 배열의 크기를 지정하여 선언하면 0으로 채워진다.
		 */
		int[] counter = new int[4]; //선언만되고 초기화 안됨(0으로 채워짐)
		
		//배열의 크기만큼 반복하면서 각 요소를 읽어온다.
		for(int i=0 ; i<answer.length ; i++) {
			/*
			if(answer[i]==1) counter[0]++; //한줄은 중괄호 생략
			else if(answer[i]==2) counter[1]++; //else if를 붙여 조건검사시간 단축

...
class Mystatic{
	...
	/*
	 정적 멤버메소드] 정적메소드 안에서는 인스턴스 멤버를 사용할수
		없다. 정적멤버만 접근가능하다.(static은 static만 호출!)
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		mystatic.instanceVar = 100; //객체생성
		System.out.println("myStatic.instanceVar="+ 
				mystatic.instanceVar); //객체를통해 참조변수로 접근
		...
		/*
		 ...
		 방법1 : 클래스명을 통해서 접근한다. 객체 생성 없이
		 	바로 접근이 가능하다.
		 */
		Mystatic.staticVar = 200; //클래스명(대문자).변수명 = static
		System.out.println("MyStatic.staticVar="+
				Mystatic.staticVar);
		
		/*
		 방법2 : 일반적인 멤버변수처럼 참조변수를 통해 접근하는것도 가능하다.
		 	단, 이경우는 정적변수를 일반적인 멤버변수로 착각할수 있으므로
		 	자바에서는 권장하지 않는다.(구지 어렵게하지마슈)
		 */
		mystatic.staticVar = 300; //인스턴스(소문자).변수 = 일반형
		System.out.println("myStatic.staticVar="+
				Mystatic.staticVar);
		...
		/*
		 정적 멤버변수는 각각의 객체마다 멤버변수가 존재하지 않고, 메소드영억에
		 하나만 생성되어 공유되므로, 아래 대입문장은 하나의 변수에 값이 할당되게
		...
		mystatic.staticMethod(); //권고사항이 아니므로 클래스명을 통해서 접근하는것이 좋음
